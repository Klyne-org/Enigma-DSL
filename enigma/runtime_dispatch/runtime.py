...
def _check_dispatch(rc: int, kernel_name: str = "", grid=None, threads=None):
    ...
    raise _gpu_error(
        ...
        kernel=kernel_name, grid=grid, threads=threads, return_code=rc,
    )
...
class MetalRuntime:
    ...
    def execute(self, compiled: CompiledKernel, inputs: List[np.ndarray],
        ...
        if not mtl_lib:
            raise _gpu_error("failed to load metallib",
                             path=compiled.metallib_path, kernel=compiled.kernel_name)

        pso = self._lib.enigma_create_pipeline(self._device, mtl_lib, compiled.kernel_name.encode())
        if not pso:
            self._lib.enigma_release(mtl_lib)
            raise _gpu_error("failed to create compute pipeline",
        ...
        try:
            for i, arr in enumerate(inputs):
                ...
                if not buf:
                    raise _gpu_error("failed to create input buffer",
                                     buffer_index=i, size_bytes=arr.nbytes,
    ...
    def prepare(self, compiled: CompiledKernel, inputs: List[np.ndarray],
        ...
        BufArr = ctypes.c_void_p * len(gpu_bufs)
        return PreparedKernel(self, pso, mtl_lib, gpu_bufs, BufArr(*gpu_bufs),
                              out_buf, output_size, compiled.kernel_name)


class PreparedKernel:
    """Pre-allocated resources for fast repeated dispatch."""

    def __init__(self, runtime, pso, mtl_lib, gpu_bufs, buf_arr,
                 out_buf, output_size, kernel_name=""):
        self._rt = runtime
