#!/usr/bin/env python3
"""Matrix multiply end-to-end test.

C[M,N] = A[M,K] * B[K,N] with K=4, using float4 dot products.
Each thread computes one output element using 2D grid indexing.
Buffers are row-major flattened 1D arrays.
"""
import os
import sys
...
M = 32
compiled = enigma.compile(matmul_k4)
...
print(f"OK  matmul {M}x{K_DIM} @ {K_DIM}x{N_DIM} -> {M}x{N_DIM}")
...
print(f"OK  matmul {M2}x{K_DIM} @ {K_DIM}x{N_DIM} -> {M2}x{N_DIM}")
